...
public class RoomAdminController {
    ...
    @GetMapping("/{roomId}")
    @Operation(summary = "Get details of a specific room",description = "Fetches details of a specific room in a hotel by ID")
    public ResponseEntity<RoomDTO> getRoomById(@PathVariable Long hotelId, @PathVariable Long roomId) {
        return ResponseEntity.ok(roomService.getRoomById(roomId));
    }

    @DeleteMapping("/{roomId}")
    @Operation(summary = "Delete a room",description = "Deletes a room from the hotel by ID")
    ...
    @PutMapping("/{roomId}")
    @Operation(summary = "Update a room",description = "Updates the details of an existing room", tags = {"Admin Inventory"})
